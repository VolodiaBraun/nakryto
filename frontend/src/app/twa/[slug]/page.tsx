...
export default function TwaPage({ params }: { params: { slug: string } }) {
  const { slug } = params;
  const { twa, isTwa } = useTelegramWebApp();
  ...
  // Set name from Telegram user once available
  useEffect(() => {
    if (tgUser && !guestForm.name) {
  ...
  const lockIdRef = useRef<string>((() => {
    ...
    const stored = sessionStorage.getItem('nakryto_twa_lock_id');
    if (stored) return stored;
    const id = uuidv4();
    sessionStorage.setItem('nakryto_twa_lock_id', id);
    return id;
  })());
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result: any = await publicApi.createBooking(slug, {
        ...
        telegramUserId: tgUser ? String(tgUser.id) : undefined,
      });
  ...
  return (
    <div className="twa-screen flex flex-col" style={{ background: 'var(--tg-bg, #f9fafb)', color: 'var(--tg-text, #111)' }}>
      ...
      <header className="twa-header sticky top-0 z-30 px-4 py-3 flex items-center gap-3"
        ...
        {/* Guest count stepper — step 0 only */}
        {step === 0 && (
        ...
        {/* Back — only when not in TWA (TWA uses BackButton) */}
        {step === 1 && !isTwa && (
                  ...
                  <div className="mb-3">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {availableSlots.map((slot: any) => (
                        <button
                          key={slot.time}
                          onClick={() => { setSelectedTime(slot.time); twa?.HapticFeedback?.selectionChanged(); }}
      ...
      {step === 1 && (
        <div className="w-full px-4 py-4">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Field label={isTwa ? 'Email (необязательно)' : 'Email'} required={!isTwa}>
              <input
                required={!isTwa}
                type="email"
              ...
              {isTwa && (
                <p className="text-xs mt-1" style={{ color: 'var(--tg-hint, #888)' }}>
                  Уведомления придут в Telegram. Email необязателен.
                </p>
              )}
            </Field>
