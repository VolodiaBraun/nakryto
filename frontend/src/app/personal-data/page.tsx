...
async function getPersonalDataPolicy(): Promise<string> {
  try {
    ...
    if (res.ok) {
      ...
      if (data.personalDataPolicy) return data.personalDataPolicy;
    }
  } catch {}
  return '';
}
...
export default async function PersonalDataPage() {
  const content = await getPersonalDataPolicy();
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b border-gray-200">
        <div className="max-w-3xl mx-auto px-6 py-4 flex items-center gap-4">
          ...
          <span className="text-gray-600 text-sm">Политика обработки персональных данных</span>
        </div>
      </header>
