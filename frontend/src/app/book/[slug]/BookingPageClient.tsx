...
import HallSelectorGrid from '@/components/HallSelectorGrid';
...
export default function BookingPageClient({ slug, hallSlug }: Props) {
  ...
  // Все залы (только когда нет конкретного hallSlug)
  const { data: allHalls = [], isLoading: hallsLoading } = useQuery<Hall[]>({
  ...
  // Конкретный зал по slug
  const { data: hallData, isLoading: hallLoading } = useQuery<{ hall: Hall; redirectTo: string | null }>({
    queryKey: ['public', slug, 'hall', hallSlug],
    queryFn: () => publicApi.getHallBySlug(slug, hallSlug!) as any,
    enabled: !!hallSlug,
    staleTime: 5 * 60_000,
  });

  // Редирект на актуальный slug при переименовании зала
  useEffect(() => {
    if (hallData?.redirectTo) {
      router.replace(`/book/${slug}/${hallData.redirectTo}`);
    }
  }, [hallData, slug, router]);

  // Итоговый список залов для отображения
  const halls: Hall[] = hallSlug
    ? (hallData?.hall ? [hallData.hall] : [])
    : allHalls;
  ...
  const selectedTable = currentHall?.tables?.find((t: any) => t.id === selectedTableId);
  const selectedTableObj: any = (currentHall?.floorPlan?.objects ?? []).find(
  ...
  const handleTableSelect = useCallback(async (tableId: string) => {
    ...
    if (prevTableId && prevTableId !== tableId) doUnlock(prevTableId, date);
    setLockError('');
    try {
  ...
  const isLoading = restaurantLoading || (hallSlug ? hallLoading : hallsLoading);
  ...
  // ─── Выбор зала (несколько залов на /book/slug без конкретного hallSlug) ─────

  if (!hallSlug && allHalls.length > 1 && allHalls[0]?.slug) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col">
        <header className="bg-white border-b border-gray-200 sticky top-0 z-30 shadow-sm">
          <div className="max-w-3xl mx-auto px-4 py-3 flex items-center gap-3">
        ...
        <HallSelectorGrid halls={allHalls} restaurantSlug={slug} />
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-30 shadow-sm">
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          {/* Кнопка "← Все залы" при переходе с selector grid */}
          {hallSlug && step === 0 && (
            <button
              onClick={() => router.push(`/book/${slug}`)}
              className="flex-shrink-0 text-sm text-gray-500 hover:text-gray-700 flex items-center gap-1"
            >
              ← Залы
            </button>
          )}
            ...
            <div className="fixed bottom-0 left-0 right-0 z-20 bg-white border-t border-gray-200 shadow-2xl">
              <div className="max-w-3xl mx-auto px-4 pt-3 pb-5">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <button onClick={handleTableDeselect} className="text-gray-400 hover:text-gray-600 p-1 -mt-1">
                    ✕
                  </button>
                </div>
